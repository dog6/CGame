...
float platformHeight = 11;
Entity platform = Entity("floor", Vector2{0, screenHeight-platformHeight}, 0);

Entity ball = Entity("ball", screenCenter);

Entity ngon = Entity("Ngon", Vector2AddValue(screenCenter, 50));
...
void Game::start(){
    ...
    rect.doesCollideWithWindow = true;

    // Create ellipse
    ball.shape = new Ellipse(16, RED); // give ball shape
    ball.body->mass = 0.3; // 0.3 kg
    ball.body->isKinematic = false;
    ball.doesCollideWithWindow = true;
    
    // Create cursor rectangle
    cursorRectangle.shape = new Rect(Vector2{12, 12}, RED); // give cursor shape

    // Create platform
    platform.shape = new Rect(Vector2{screenWidth, platformHeight}, DARKGRAY);

    // Create Ngon (triangle)
    ngon.shape = new Ngon(3, 32, GREEN);
    ngon.body->mass = 10;
    ngon.body->isKinematic = false;
    ngon.doesCollideWithWindow = true;

    // Set scene gravity
    // demoScene.setGravity(Vector2{0, 9.81});

    // Or add multiple entities at once instead
    demoScene.addEntities(vector<Entity*>{ &tilemap, &ball, &cursorRectangle, &rect, /*&platform,*/ &ngon});
